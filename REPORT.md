...
The model follows a **two-stage detection pipeline**. In the first stage, candidate object regions are proposed using a **Region Proposal Network (RPN)**. In the second stage, these regions are classified into object categories and their bounding boxes are refined.
...
The backbone network is responsible for extracting meaningful features from the input image.

In this model, the backbone is a **Swin Transformer**, specifically the **Swin-Tiny architecture**. Unlike traditional convolutional neural networks, the Swin Transformer uses **self-attention mechanisms** to capture relationships between different regions of the image.

A key feature of the Swin Transformer is the use of **shifted window self-attention**, which computes attention within local windows and shifts these windows between layers. This design allows the model to efficiently capture both **local and global contextual information** while maintaining computational efficiency.

The Swin backbone produces a set of **hierarchical feature maps** at different spatial resolutions. These feature maps contain rich semantic information that is useful for object detection.

---

### 4.2 Feature Pyramid Network (FPN)

Objects in real-world images appear at different scales. To handle this variation, the model uses a **Feature Pyramid Network (FPN)**.

The FPN combines feature maps from multiple levels of the backbone to create a **multi-scale representation of the image**. Higher-resolution feature maps help detect small objects, while lower-resolution feature maps capture larger objects with more context.

By merging features across scales, the FPN improves detection performance for objects of varying sizes.

---

### 4.3 Region Proposal Network (RPN)

The Region Proposal Network is responsible for generating candidate object regions from the feature maps produced by the FPN.

The RPN slides a small network over the feature maps and predicts **anchor boxes** at each spatial location. Each anchor box represents a potential object location with a predefined scale and aspect ratio.

For each anchor, the RPN predicts:

- an **objectness score** indicating whether the region contains an object
- **bounding box offsets** that refine the anchor to better fit the object

The top candidate proposals generated by the RPN are passed to the next stage of the detection pipeline.

---

### 4.4 ROI-Based Detection Head

The second stage of the Faster R-CNN architecture processes the region proposals generated by the RPN.

First, the **ROI Align operation** extracts fixed-size feature maps from the feature pyramid for each proposed region. These features are then passed through the detection head.

The detection head performs two tasks:

1. **Object classification** – predicting the object category for each region.
2. **Bounding box regression** – refining the coordinates of the predicted bounding boxes.
...
To address class imbalance in the BDD100K dataset, a **weighted classification loss** is used in the ROI detection head. Class-specific weights are applied during the cross-entropy loss computation to reduce the dominance of frequent classes such as cars and increase the importance of less frequent classes.

In addition to the classification loss, the model also uses a **Smooth L1 loss** for bounding box regression to improve localization accuracy.

---

### 4.6 Overall Detection Pipeline

The complete detection process can be summarized as follows:

1. The input image is passed through the **Swin Transformer backbone** to extract hierarchical feature maps.
2. The **Feature Pyramid Network** combines these feature maps to create multi-scale representations.
3. The **Region Proposal Network** generates candidate object regions.
4. **ROI Align** extracts region-specific features.
5. The **detection head** performs object classification and bounding box regression.

This two-stage architecture allows the model to achieve **high detection accuracy and precise object localization**, making it well suited for complex driving scenes such as those found in the BDD100K dataset.
